...
Вариант № 13:
Определить количество взрослых(>=18), севших в порту Шербур,
...
for i in sherbur_list:
    if i[5] != '' and i[5] != ' ' and (position_age_list[0] <= float(i[5]) <= position_age_list[-1]):
        cnt_sherbur_sred_age += 1
        if i[1] == '1':
            cnt_sherbur_sred_age_alive += 1

print(f'\nКоличество взрослых, севших в порту Шербур в возрасте от {position_age_list[0]} до {position_age_list[-1]}: {cnt_sherbur_sred_age}')
